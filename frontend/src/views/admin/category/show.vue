<template lang="pug">
    v-row(justify="center")
        v-col.py-0(cols="12")
            //- Title
            h3.secondary--text {{ $t('admin.category.show') }}
                            ...
                            v-col.py-0.white--text(cols="12")
                                ...
                                div
                                    span.font-weight-bold
                                        | {{ $t('admin.category.fields.created_at') }}:
                                    | {{ record.created_at | moment('LLL') }}
                                //- Created by
                                div
                                    span.font-weight-bold
                                        | {{ $t('admin.category.fields.created_by') }}:
                                    | {{ record.created_by.name }}
                                //- Updated at
                                div(v-if="record.updated_at")
                                    span.font-weight-bold
                                        | {{ $t('admin.category.fields.updated_at') }}:
                                    | {{ record.updated_at | moment('LLL') }}
                                //- Updated by
                                div(v-if="record.updated_by")
                                    span.font-weight-bold
                                        | {{ $t('admin.category.fields.updated_by') }}:
...
export default {
    ...
    created() {
        ...
        axios.get(`/api/categories/${this.id}`).then((response) => {
            this.loading = false;
